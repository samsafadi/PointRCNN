...
def gen_sparse_points_all(args):
    ...
    data_idx_list = sorted(
        [x.strip() for x in os.listdir(args.pl_path) if x[-3:] == 'bin'])

    for data_idx in tqdm.tqdm(data_idx_list):
        sparse_points = gen_sparse_points(
            os.path.join(args.pl_path, data_idx), args)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--pl_path', default='/scratch/datasets', help='pseudo-lidar path')
    parser.add_argument(
        '--sparse_pl_path', default='/scratch/datasets', help='sparsed pseudo lidar path')
